...
def range_slider(id=None, min:int=2019 , max:int=2023, step:int = 1, value: Sequence[int]=[2019,2023], marks = None, width = "100%"):
    return html.Div([
...
def table(id, df, style=None):
    _dt = df.to_dict('records')
    if not style:
        style = dict()
    return html.Div([
            dash_table.DataTable(id=id, data=_dt)
    ], style=style
    )
